...
class EchoClient : public quic::QuicSocket::ConnectionCallback,
 ...
 public:
  EchoClient(const std::string& host, uint16_t port, bool prEnabled = false)
      : host_(host), port_(port), prEnabled_(prEnabled),networkThread_("EchoClientThread"){
        start();
      }

  void readAvailable(quic::StreamId streamId) noexcept override { //client read message [pingcoool]
  ...
  void start() {
    ...
    evb_->runInEventBaseThreadAndWait([&] {
      ...
      printf("%d,%p\n",__LINE__,quicClient_.get());
    });
    //↑对quicClient进行初始化
  }
  void sendBytes(char *buffer, int len){
    //↓从控制台拿数据进行传输
  ...
  void SendMessageCallback(std::string message){
        ...
        LOG(INFO) <<"client after"<<streamId;
      quicClient_->setReadCallback(streamId, this);
        LOG(INFO) <<"fuck";
        LOG(INFO) <<"size"<<message.size();
        if (this->prEnabled_) {
          quicClient_->setDataExpiredCallback(streamId, this);
        }
